...
io.on('connection', function(socket){
    ...
    socket.on('all messages', function (data) {
        mongo.connect(db_connect, function (err, db) {
            ...
            collection.aggregate([ {$match: {id_users: {$in: [id_user]}}}, {$group: {_id: { id_users: "$id_users" }, text: { $last: "$text" }, sender: { $last: "$sender" }, receiver: { $last: "$receiver" }, reviewed: { $last: "$reviewed" }, date: { $last: "$date" }}}, {$sort : { date : -1 }} ], function (err, result) {
                if (err) {
                } else {
                    var companion = [];
