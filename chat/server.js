...
io.on('connection', function(socket){
    ...
    socket.on('all messages', function (data) {
        mongo.connect(db_connect, function (err, db) {
            ...
            collection.aggregate([ {$match: {id_users: {$in: [id_user]}}}, {$sort : { date : -1 }}, {$group: {_id: { id_users: "$id_users" }, text: { $last: "$text" }, sender: { $last: "$sender" }, receiver: { $last: "$receiver" }, reviewed: { $last: "$reviewed" }, date: { $last: "$date" }}} ], function (err, result) {
                if (err) {
                } else {
                    var companion = [];
